...
episode, score_avg, score_max = 0, 0, -np.inf
...
class A3CAgent():
    def __init__(self, action_size, env_name, 
        ...
        # 텐서보드 설정
        self.writer = tf.summary.create_file_writer(f'summary/{self.env_name}')
        # 학습된 글로벌신경망 모델을 저장할 경로 설정
        self.model_path = os.path.join(os.getcwd(), f'save_model_{self.env_name}', 'model')
    ...
    def train(self):
        ...
        runners = [Runner(self.action_size, self.state_size,
                          ...
                          self.discount_factor, self.t_max,
                          self.num_episode,self.env_name,
